...
from ..build import q06_analize_sentiment
from inspect import getfullargspec
import pandas
import numpy as np

data=pandas.read_csv('...data/data.csv')
data['SA'] = np.array([q06_analize_sentiment(tweet) for tweet in data['Tweets']])

class TestRead_csv_data_to_df(TestCase):
    def test_args(self):
        arg = getfullargspec(q06_analize_sentiment).args
